        ...
        try:
            with DBconnection() as mc_afficher:
                if order_by == "ASC" and id_genre_sel == 0:
                    strsql_genres_afficher = """SELECT id, id AS id_genre, name AS intitule_genre, created_at AS date_ins_genre FROM t_folders ORDER BY id ASC"""
                    mc_afficher.execute(strsql_genres_afficher)
                elif order_by == "ASC":
                    valeur_id_genre_selected_dictionnaire = {"value_id_genre_selected": id_genre_sel}
                    strsql_genres_afficher = """SELECT id, id AS id_genre, name AS intitule_genre, created_at AS date_ins_genre FROM t_folders WHERE id = %(value_id_genre_selected)s"""

                    mc_afficher.execute(strsql_genres_afficher, valeur_id_genre_selected_dictionnaire)
                else:
                    strsql_genres_afficher = """SELECT id, id AS id_genre, name AS intitule_genre, created_at AS date_ins_genre FROM t_folders ORDER BY id DESC"""
    ...
    if request.method == "POST":
        try:
            if form.validate_on_submit():
                ...
                strsql_insert_genre = """INSERT INTO t_folders (id,name) VALUES (NULL,%(value_intitule_genre)s) """
...
def genre_update_wtf():
    ...
    try:
        ...
        if request.method == "POST" and form_update.submit.data:
            ...
            str_sql_update_intitulegenre = """UPDATE t_folders SET name = %(value_name_genre)s, 
            created_at = %(value_date_genre_essai)s WHERE id = %(value_id_genre)s """
            with DBconnection() as mconn_bd:
            ...
            # Opération sur la BD pour récupérer "id" et "name" de la "t_folders"
            str_sql_id_genre = "SELECT id AS id_genre, name AS intitule_genre, created_at AS date_ins_genre FROM t_folders " \
    ...
    try:
        ...
        if request.method == "POST" and form_delete.validate_on_submit():
            ...
            if form_delete.submit_btn_del.data:
                ...
                str_sql_unlink_files = """UPDATE t_files SET folder_id = NULL WHERE folder_id = %(value_id_genre)s"""
                str_sql_delete_idgenre = """DELETE FROM t_folders WHERE id = %(value_id_genre)s"""
                with DBconnection() as mconn_bd:
        ...
        if request.method == "GET":
            ...
            # Requête qui affiche tous les fichiers qui sont dans le dossier que l'utilisateur veut effacer
            str_sql_genres_films_delete = """SELECT t_files.id AS id_film, t_files.name AS nom_film, t_folders.id AS id_genre, t_folders.name AS intitule_genre FROM t_files 
            ...
            with DBconnection() as mydb_conn:
                ...
                # Opération sur la BD pour récupérer "id" et "name" de la "t_folders"
                str_sql_id_genre = "SELECT id AS id_genre, name AS intitule_genre FROM t_folders WHERE id = %(value_id_genre)s"
