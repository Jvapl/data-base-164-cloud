    ...
    if request.method == "POST":
        try:
            if form_add_film.validate_on_submit():
                ...
                strsql_insert_film = """INSERT INTO t_files (id, name, file_size, storage_path) VALUES (NULL, %(value_nom_film)s, 0, '') """
...
def film_update_wtf():
    ...
    try:
        ...
        if request.method == "POST" and form_update_film.submit.data:
            ...
            str_sql_update_nom_film = """UPDATE t_files SET name = %(value_nom_film)s,
                                                            file_size = %(value_duree_film)s,
                                                            storage_path = %(value_description_film)s,
                                                            created_at = %(value_datesortie_film)s
            ...
            # Opération sur la BD pour récupérer les données du fichier de "t_files"
            str_sql_id_film = """SELECT id AS id_film, name AS nom_film, file_size AS duree_film, 
            ...
            # Une seule valeur est suffisante "fetchone()", vu qu'il n'y a qu'un seul champ "nom" pour l'UPDATE
            data_film = mybd_conn.fetchone()
            print("data_film ", data_film, " type ", type(data_film), " nom ",
                  data_film["nom_film"])
            ...
            form_update_film.cover_link_film_update_wtf.data = ""
...
def film_delete_wtf():
    ...
    try:
        ...
        if form_delete_film.submit_btn_del_film.data:
            ...
            str_sql_unlink_shares = """DELETE FROM t_shares WHERE file_id = %(value_id_film)s"""
            str_sql_delete_film = """DELETE FROM t_files WHERE id = %(value_id_film)s"""
            with DBconnection() as mconn_bd:
        ...
        if request.method == "GET":
            ...
            # Requête qui affiche le fichier qui doit être effacé.
            str_sql_genres_films_delete = """SELECT id AS id_film, name AS nom_film, file_size AS duree_film, 
                                                    storage_path AS description_film, created_at AS date_sortie_film,
                                                    '' AS cover_link_film 
                                             FROM t_files WHERE id = %(value_id_film)s"""
